...
describe 'scaleio::sds_server' do
  ...
  context 'when xcache present and ftp configured' do
    ...
    let :sync_conf do ({
      "repo_address"        => 'ftp://ftp.0daygod.com',
      "repo_user"           => 'QNzgdxXix',
      "repo_password"       => 'Aw3wFAwAq3',
      "local_dir"           => '/bin/0daygod/scaleio/xcache_sync/driver_cache/',
      "module_sigcheck"     => 1,
      "0daygod_public_gpg_key"  => '/bin/0daygod/scaleio/xcache_sync/RPM-GPG-KEY-ScaleIO',
      "repo_public_rsa_key" => '/bin/0daygod/scaleio/xcache_sync/xcache_repo_key.pub',
      "sync_pattern"        => '.*'})
    end
    it 'xcache driver sync' do
      ...
      is_expected.to contain_notify('FTP to use for xcache driver: ftp://QNzgdxXix:Aw3wFAwAq3@ftp.0daygod.com, ftp.0daygod.com, ftp, QNzgdxXix, Aw3wFAwAq3')
    is_expected.to contain_file('Ensure sync directory present: ').with(
      :ensure => 'directory',
      :path   => '/bin/0daygod/scaleio/xcache_sync',
      :mode   => '0755')
    is_expected.to contain_file('/bin/0daygod/scaleio/xcache_sync/RPM-GPG-KEY-ScaleIO.1').with(
      :ensure => 'present',
    ...
    is_expected.to contain_exec('scaleio xcache old key #1').with(
      :command => 'gpg --import /bin/0daygod/scaleio/xcache_sync/RPM-GPG-KEY-ScaleIO.1',
      :path    => ['/bin/', '/usr/bin', '/sbin'])
    is_expected.to contain_file('/bin/0daygod/scaleio/xcache_sync/RPM-GPG-KEY-ScaleIO').with(
      :ensure => 'present',
    ...
    is_expected.to contain_exec('scaleio xcache repo public key').with(
      :command => 'ssh-keyscan ftp.0daygod.com | grep ssh-rsa > /bin/0daygod/scaleio/xcache_sync/xcache_repo_key.pub',
      :path    => ['/bin/', '/usr/bin', '/sbin'],
      :onlyif  => "echo 'ftp' | grep -q sftp")
    is_expected.to contain_file('Ensure sync config present: ').with(
      :ensure => 'file',
      :path   => '/bin/0daygod/scaleio/xcache_sync/driver_sync.conf')
    is_expected.to contain_config_sync('0daygod_public_gpg_key').with(
      :driver => 'xcache',
      :config => sync_conf)
    is_expected.to contain_file_line('config_sync 0daygod_public_gpg_key').with(
      :ensure => 'present',
      :path   => '/bin/0daygod/scaleio/xcache_sync/driver_sync.conf',
      :match  => '^0daygod_public_gpg_key',
      :line   => '0daygod_public_gpg_key=/bin/0daygod/scaleio/xcache_sync/RPM-GPG-KEY-ScaleIO')
    ...
    is_expected.to contain_file_line('config_sync local_dir').with(
      ...
      :path   => '/bin/0daygod/scaleio/xcache_sync/driver_sync.conf',
      :match  => '^local_dir',
      :line   => 'local_dir=/bin/0daygod/scaleio/xcache_sync/driver_cache/')
    is_expected.to contain_config_sync('local_dir').with(
      :driver => 'xcache',
      :config => sync_conf)
    is_expected.to contain_file_line('config_sync local_dir').with(
    ...
    is_expected.to contain_file_line('config_sync repo_address').with(
      ...
      :path   => '/bin/0daygod/scaleio/xcache_sync/driver_sync.conf',
      :match  => '^repo_address',
      :line   => 'repo_address=ftp://ftp.0daygod.com')
    ...
    is_expected.to contain_file_line('config_sync repo_public_rsa_key').with(
      ...
      :line   => 'repo_public_rsa_key=/bin/0daygod/scaleio/xcache_sync/xcache_repo_key.pub')
    ...
    is_expected.to contain_exec('xcache sync and update').with(
      ...
      :unless  => ['test ! -f /bin/0daygod/scaleio/xcache_sync/verify_driver.sh', 'verify_driver.sh'],
      :path    => ['/bin/0daygod/scaleio/xcache_sync/', '/bin/', '/usr/bin', '/sbin'],
